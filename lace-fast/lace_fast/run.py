#!/usr/bin/env python
"""lace-fast entry point — parallelised SuperTranscript construction.
...
* **IO1**    – in-memory cluster dispatch via ``ProcessPoolExecutor.starmap``
...
from lace_fast import __version__
from lace_fast.build_supertranscript import (
    ClusterResult,
    get_annotation_line,
    super_tran,
)
...
def split_and_build(
    ...
    *,
    ...
    with open(genome_path) as fh:
        for line in fh:
            if line.startswith(">"):
                ...
                transcripts[current] = ""
                # Assign to cluster (skip singletons & unmapped)
    ...
    with (
        ...
        # Multi-transcript clusters (in original input order for reproducibility)
...
def main(args: list[str] | None = None) -> None:
    """Main entry point for lace-fast."""
    print(BANNER)
